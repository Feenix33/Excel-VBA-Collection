...
Function ProcessSheetName(inputSheetName As String) As String
    ...
    If pos > 0 Then
       newSheetName = Left(newSheetName, pos - 1) + "Milestone" + Right(newSheetName, Len(newSheetName) - pos - 1)
    End If
...
Sub cmeSetDefaultStyles()
    Dim aTable
    Dim pvtTbl As Excel.PivotTable
    Dim bTripped ' one error handler
    

    On Error GoTo ErrorHandler
    bTripped = False
    aTable = ActiveCell.ListObject.Name
    ActiveSheet.ListObjects(aTable).TableStyle = "TableStyleMedium18"
    Exit Sub
ErrorHandler:
    
    If bTripped Then Exit Sub
    bTripped = True
    
    Set pvtTbl = ActiveCell.PivotCell.PivotTable
    pvtTbl.TableStyle2 = "PivotStyleMedium11"
End Sub
